...
#'
#' \code{find_critical_points_curve} Returns a list of the values at which the ec curve changes, for every direction specified.
#'
...
find_critical_points_curve=function(directions, off, len){
    critical_points=list()

    # For each direction in the cone of directions generated by generate_perturbed_directions
    for (i in 1:5){

        # generate the ec curve in this direction
        direction=directions[[i]]
        vertex_function=off$Vertices%*%direction
        ec_curve=compute_discrete_ec_curve(off,vertex_function,len)
        num_t=dim(ec_curve)[1]

        crit_points=c()
        interval=c()

        # iterate over those indices, keeping track of the indices at which the ec changes,
        ...
        # Does this pose in issue ?  one entry of the list might be empty.
...
find_critical_points_multiple_directions=function(dir,off,steps,phi){
    critical_points=list()
    total_directions=dim(dir)[1]
    for (i in 1:total_directions){
        direction=dir[i,]
        crit_points=find_critical_point_coordinates(direction = direction,off = off,steps = steps,phi = phi)
...
plot_points=function(points,point_size=5,point_color='red',alpha=1){
  rgl.points(points)
  rgl.points(points,col=point_color,alpha=alpha,size=point_size)
}
...
plot_complex_with_points=function(points,vertices,n=100,main_color='white',crit_color='blue',alpha_one=0.5,alpha_two=0.3,off_file,axes=FALSE,labels=FALSE,plot_points=FALSE,hull=TRUE,point_size=10){
  ...
  if (hull==TRUE){
    ...
    plot_points(points,point_size=point_size,point_color=crit_color,alpha=alpha)
  }
}
